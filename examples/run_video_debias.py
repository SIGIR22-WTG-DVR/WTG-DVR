...
flags.DEFINE_boolean('remove_duration_feature', True, 'Whether to remove duration from input features.')
...
def test_model(flags_obj, cfg, model, data_splits, model_input, target, watchtime_target, gain_target, like, mean_play, std_play):
...
def main(argv):
    ...
    if not flags_obj.test_ckpt:
        ...
        result = test_model(flags_obj, cfg, model, data_splits, model_input, target, watchtime_target, gain_target, like, mean_play, std_play)
